...
Repository coding standards are documented in [`.github/instructions`](.github/instructions), including:

- comments that explain intent and decisions rather than restating code;
- TSDoc/JSDoc requirements for exported functions in `db/` and `src/lib/`;
- documented `Props` contracts for reusable Astro components; and
- TypeScript formatting and explicit module-boundary types enforced by ESLint.
